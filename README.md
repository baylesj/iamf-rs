...
4. **Reconstruction & rendering** — *(core done)* substream→channel
   reconstruction (single-layer channel layouts, ambisonics mono and
   projection), rendering via gain matrices extracted from libiamf v1.1.0
   (`tools/extract_matrices.py`; M2M and HOA→M for all layout pairs),
   element/output mix gains, multi-element mixing, `iamfdec -o out.wav -s N`.
   Validated against reference rendered WAVs: same-layout paths bit-exact,
   cross-layout within ±1..3 LSB (references are generated by iamf-tools,
   whose arithmetic differs at the last bit). Still open: scalable
   multi-layer demixing + recon gain, layer output_gain, animated parameter
   gains, loudness normalization, peak limiter.
